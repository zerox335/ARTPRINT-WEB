...
const printExclusionSchema = rectangleSchema.and(z.object({
...
const mockupSchema = z.object({
  ...
  printAreas: z.array(printAreaSchema).min(1).max(6),
});
...
export const catalogProductInputSchema = z.object({
  ...
  galleryAssetIds: z.array(z.string().min(1).max(120)).min(1).max(20),
  mockups: z.array(mockupSchema).max(8),
}).superRefine((product, context) => {
  if (product.customizable && product.mockups.length === 0) {
    context.addIssue({ code: "custom", path: ["mockups"], message: "Un producto personalizable necesita al menos un mockup" });
  }
  if (product.productType === "CASE" && !product.brand) {
    context.addIssue({ code: "custom", path: ["brand"], message: "Indica la marca de la carcasa" });
  }
