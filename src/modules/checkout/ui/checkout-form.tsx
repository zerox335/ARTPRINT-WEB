...
import { CheckCircle2, ChevronRight, LockKeyhole, MapPin, UserRound } from "lucide-react";
...
export function CheckoutForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const payload = {
        ...
        shipping: { recipient: String(form.get("name")), line1: String(form.get("line1")), line2: String(form.get("line2") || "") || undefined, city: String(form.get("city")), department: String(form.get("department")), notes: String(form.get("notes") || "") || undefined },
      };
  ...
  return (
    <form className="container checkout-layout" onSubmit={submit}>
      <div className="checkout-main"><div className="checkout-heading"><p className="eyebrow">Finalizar compra</p><h1>¿A dónde llevamos tu idea?</h1><p>Sesión de <strong>{user.email}</strong></p></div><fieldset><legend><UserRound size={18} /> Datos de contacto</legend><div className="form-grid"><label>Nombre completo<input required name="name" minLength={2} maxLength={100} defaultValue={user.name} autoComplete="name" /></label><label>Correo<input required name="email" type="email" defaultValue={user.email} autoComplete="email" /></label><label>Teléfono<input required name="phone" inputMode="tel" pattern="[+0-9 -]{7,20}" autoComplete="tel" placeholder="300 123 4567" /></label></div></fieldset><fieldset><legend><MapPin size={18} /> Entrega</legend><div className="form-grid"><label className="full-field">Dirección<input required name="line1" minLength={5} maxLength={160} autoComplete="address-line1" placeholder="Calle, carrera, número" /></label><label className="full-field">Complemento<input name="line2" maxLength={160} autoComplete="address-line2" placeholder="Apartamento, oficina (opcional)" /></label><label>Ciudad<input required name="city" minLength={2} maxLength={100} autoComplete="address-level2" /></label><label>Departamento<input required name="department" minLength={2} maxLength={100} autoComplete="address-level1" /></label><label className="full-field">Notas de producción o entrega<textarea name="notes" maxLength={500} rows={3} /></label></div></fieldset>{error && <p className="inline-alert" role="alert">{error}</p>}</div>
      <aside className="order-summary checkout-order"><div className="checkout-secure"><LockKeyhole size={18} /><span>Checkout seguro</span></div><h2>Tu pedido</h2><div className="mini-lines">{lines.map((line) => <div key={line.id}><span>{line.quantity}× {line.productName}<small>{line.variant.name}</small></span><strong>{formatMoney(line.quote.total)}</strong></div>)}</div><div className="summary-row"><span>Subtotal</span><span>{formatMoney(total)}</span></div><div className="summary-row"><span>Envío</span><span>{shipping ? formatMoney(shipping) : "Gratis"}</span></div><div className="summary-grand"><span>Total</span><strong>{formatMoney(total + shipping)}</strong></div><button className="button button-gradient button-block" disabled={submitting}>{submitting ? "Creando pedido…" : "Ir al pago"} <ChevronRight size={17} /></button><p className="legal-note"><CheckCircle2 size={15} /> El pago solo se confirma después del webhook auténtico de la pasarela.</p></aside>
    </form>
  );
}
