...
export function ProductCustomizer({ product, initialVariantId, initialQuantity, previewOnly = false }: { product: ProductView; initialVariantId?: string; initialQuantity?: number; previewOnly?: boolean }) {
  ...
  return (
    <div className={`customizer-layout container${isPhoneCase ? " case-customizer-layout" : ""}`}>
      <section className="customizer-workspace">
        ...
        <DesignEditor product={product} area={activeArea} variantColor={variant?.colorHex} previewOnly={previewOnly} onChange={handleElements} />
      </section>
