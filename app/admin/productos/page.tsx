  ...
  return (
    <section className="container admin-page">
      ...
      <div className="admin-table-card">
        ...
        <div className="admin-table-wrap"><table><thead><tr><th>Producto</th><th>Categoría</th><th>Precio</th><th>Contenido</th><th>Estado</th><th>Acción</th></tr></thead><tbody>{products.map((product) => { const metadata = product.metadata && typeof product.metadata === "object" && !Array.isArray(product.metadata) ? product.metadata as Record<string, unknown> : {}; const isCaseReference = product.status === "DRAFT" && product.category.slug === "carcasas" && metadata.mockupStatus === "REFERENCE_ONLY"; return <tr key={product.id}><td><strong>{product.name}</strong><small>{product.slug}</small></td><td>{product.category.name}</td><td>{formatMoney(product.basePrice)}</td><td>{product._count.variants} var. · {product._count.images} fotos · {product._count.mockups} mockups</td><td><span className={`status-chip status-${product.status.toLocaleLowerCase()}`}>{isCaseReference ? "SIN MOCKUP" : product.status}</span></td><td>{product.status === "ACTIVE" ? <Link className="admin-table-link" href={`/productos/${product.slug}`} target="_blank">Abrir <ExternalLink size={13} /></Link> : isCaseReference ? <Link className="admin-table-link" href={`/admin/productos/${product.id}`}>Configurar mockup</Link> : <span className="admin-muted">Privado</span>}</td></tr>; })}</tbody></table></div>
      </div>
