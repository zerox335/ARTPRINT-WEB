...
import { DesignProofPublisher } from "@/src/modules/admin/ui/design-proof-publisher";
...
export default async function AdminPage() {
  ...
  const [todaySales, monthSales, counts, orders, customerCount] = await Promise.all([
    ...
    prisma.order.findMany({ orderBy: { createdAt: "desc" }, take: 12, include: { user: { select: { name: true, email: true } }, items: { include: { proofs: { orderBy: { version: "desc" }, take: 1 } } } } }),
  ...
  const designItems = orders.flatMap((order) => ["PAID", "DESIGN_REVIEW", "WAITING_CUSTOMER_APPROVAL"].includes(order.status) ? order.items.map((item) => ({ order, item, product: item.productSnapshot as { name?: string } })) : []);
  return <section className="container admin-page"><div className="admin-top"><div><p className="eyebrow">Operación ArtPrint</p><h1>Hola, {user.name.split(" ")[0]}</h1><p>Una vista clara de lo que necesita atención hoy.</p></div><Link className="button button-gradient" href="/admin/productos"><Plus size={17} /> Nuevo producto</Link></div><div className="admin-nav"><Link className="active" href="/admin">Resumen</Link><a href="#pedidos">Pedidos</a><a href="#pruebas">Diseños</a><Link href="/admin/productos">Productos</Link><Link href="/disenos-listos" target="_blank">Diseños listos</Link></div><div className="metric-grid">{cards.map(([Icon,label,value]) => { const C = Icon; return <div className="metric-card" key={label}><C size={20} /><span>{label}</span><strong>{value}</strong></div>; })}</div><div className="admin-table-card" id="pedidos"><div className="admin-table-head"><div><h2>Pedidos recientes</h2><p>Estado comercial y de producción</p></div><span>{orders.length} visibles</span></div><div className="admin-table-wrap"><table><thead><tr><th>Pedido</th><th>Cliente</th><th>Total</th><th>Estado</th><th>Fecha</th><th>Acción</th></tr></thead><tbody>{orders.map((order) => <tr key={order.id}><td><strong>{order.number}</strong></td><td>{order.user?.name ?? "Cliente"}<small>{order.user?.email}</small></td><td>{formatMoney(order.grandTotal)}</td><td><span className={`status-chip status-${order.status.toLocaleLowerCase()}`}>{order.status.replaceAll("_", " ")}</span></td><td>{new Intl.DateTimeFormat("es-CO", { dateStyle: "medium" }).format(order.createdAt)}</td><td><OrderStatusControl orderId={order.id} status={order.status} /></td></tr>)}</tbody></table></div></div><div className="admin-table-card proof-queue" id="pruebas"><div className="admin-table-head"><div><h2>Pruebas para aprobación</h2><p>Publica el montaje final que verá el cliente.</p></div><span>{designItems.length} pendientes</span></div>{designItems.length ? <div className="proof-queue-list">{designItems.map(({ order, item, product }) => <div className="proof-queue-item" key={item.id}><div><strong>{order.number}</strong><span>{order.user?.name ?? "Cliente"}</span></div><DesignProofPublisher orderItemId={item.id} productName={product.name ?? "Producto personalizado"} currentVersion={item.proofs[0]?.version ?? 0} /></div>)}</div> : <p className="admin-empty-copy">No hay diseños pendientes de revisión.</p>}</div></section>;
}
