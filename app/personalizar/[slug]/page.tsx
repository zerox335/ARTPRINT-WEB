...
export default async function CustomizePage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ variant?: string; quantity?: string }> }) {
  ...
  return <ProductCustomizer product={product} initialVariantId={query.variant} initialQuantity={query.quantity ? Number(query.quantity) : undefined} previewOnly={env.DEMO_MODE === "true" || !env.DATABASE_URL} />;
}
