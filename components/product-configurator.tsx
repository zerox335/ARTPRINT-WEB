...
export function ProductConfigurator({ product }: { product: ProductView }) {
  ...
  return (
    <div>
      ...
      <div className="product-actions"><button className="button button-secondary" onClick={addBasicProduct} disabled={!quote}>{added ? <><Check size={18} /> Agregado</> : "Agregar sin diseño"}</button><Link className="button button-gradient" href={`/personalizar/${product.slug}?variant=${variantId}&quantity=${quantity}`}><Sparkles size={18} /> Personalizar ahora</Link></div>
